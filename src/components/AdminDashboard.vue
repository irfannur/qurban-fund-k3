<template>
  <div class="space-y-6">
    <!-- Header Admin -->
    <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-6 text-white shadow-md flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      <div>
        <p class="text-xs uppercase tracking-wider text-slate-400 font-semibold mb-1">Panel Kendali Admin</p>
        <h1 class="text-2xl font-bold">Rekap Keseluruhan Qurban</h1>
      </div>
      <!-- Filter Bulan Global -->
      <div class="bg-slate-700/50 p-1.5 rounded-xl border border-slate-700 w-full md:w-auto">
        <select v-model="selectedBulanGlobal" class="bg-transparent text-white font-medium text-sm px-3 py-1.5 focus:outline-none w-full cursor-pointer">
          <option v-for="bln in listBulan" :key="bln" :value="bln" class="text-slate-900">{{ bln }}</option>
        </select>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
      ...
      <div class="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
        <div>
          <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">Total Kas Terkumpul</span>
          <h3 class="text-2xl font-extrabold text-emerald-600 mt-2">{{ formatRupiah(totalKasSemuaBulan) }}</h3>
        </div>
    ...
    <div class="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
      <div class="p-5 border-b border-slate-100">
        <h2 class="font-bold text-slate-700 flex items-center gap-2">
          ...
          Daftar Tabungan Anggota ({{ selectedBulanGlobal }})
        </h2>
      </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-slate-50 text-slate-400 text-xs font-semibold uppercase border-b border-slate-100">
              ...
              <td class="py-3.5 px-5 text-center flex items-center justify-center gap-2">
                ...
                <button @click="shareLink(user.nama, user.token)" 
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 10.742l4.632-2.316m0 0a3 3 0 102.217-4.217 3 3 0 00-2.217 4.217zm-4.632 6l4.632 2.316m0 0a3 3 0 102.217-4.217 3 3 0 00-2.217 4.217z" />
                  </svg>
                  Share
                </button>
              </td>
    ...
    <div v-if="isDrawerOpen" class="fixed inset-0 z-50 overflow-hidden" role="dialog" aria-modal="true">
      ...
      <div class="absolute inset-x-0 bottom-0 max-h-[90vh] bg-slate-50 rounded-t-[2rem] shadow-2xl flex flex-col transition-transform transform duration-300 translate-y-0 overflow-hidden">
        
        <!-- Handle Drawer (Untuk visual slide) -->
        <div class="flex justify-center py-3 bg-white border-b border-slate-100 cursor-pointer" @click="tutupDrawer">
        ...
        <div class="px-6 py-4 bg-white border-b border-slate-100 flex justify-between items-center">
          <div>
            <span class="text-[10px] uppercase tracking-wider font-bold text-slate-400">Rincian Tabungan Anggota</span>
            <h3 class="text-lg font-bold text-slate-800">{{ selectedUserDetail?.nama }}</h3>
          </div>
          <!-- Tombol Close -->
          <button @click="tutupDrawer" class="p-2 hover:bg-slate-100 rounded-full text-slate-400 hover:text-slate-600 transition">
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div class="bg-gradient-to-r from-emerald-600 to-teal-700 text-white rounded-2xl p-5 shadow-sm flex justify-between items-center">
            <span class="text-xs font-semibold uppercase tracking-wider">Total Tabungan Terkumpul</span>
            <span class="text-xl font-extrabold">{{ formatRupiah(selectedUserDetail?.total_semua_bulan || 0) }}</span>
          </div>
          ...
          <div class="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
            ...
            <div class="p-4 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              ...
              <div class="bg-slate-100 p-1 rounded-lg border border-slate-200/60 w-full sm:w-auto">
                <select v-model="drawerSelectedBulan" class="bg-transparent text-slate-700 font-semibold text-[11px] px-2 py-1 focus:outline-none w-full cursor-pointer">
            ...
            <div class="px-4 py-3 bg-emerald-50/40 border-b border-slate-100 flex justify-between items-center text-xs">
              <span class="text-slate-500 font-semibold">Total Bulan Ini ({{ drawerSelectedBulan }}):</span>
            ...
            <table class="w-full text-left border-collapse">
              ...
              <tbody class="divide-y divide-slate-50 text-xs">
                <tr v-for="setor in setoranBulanTerpilihDrawer" :key="setor.minggu" class="hover:bg-slate-50/30">
                  ...
                  <td class="py-2.5 px-4 text-right font-bold text-slate-700">{{ formatRupiah(setor.nominal) }}</td>
                </tr>
                <tr v-if="setoranBulanTerpilihDrawer.length === 0">
                  <td colspan="3" class="py-4 text-center text-slate-400 italic">Belum ada catatan setoran.</td>
                </tr>
              </tbody>
            </table>
          </div>
  ...
  setup(props) {
    ...
    onMounted(async () => {
      try {
        ...
        console.error("Gagal memuat data admin dinamis:", err)
      }
    })
    ...
    return { 
      ...
      // Export Drawer States & Functions
      isDrawerOpen,
      selectedUserDetail,
      drawerSelectedBulan,
      bukaDetailUser,
