<template>
    <div class="space-y-6">
        <!-- Header Welcome User -->
        <div
            class="bg-gradient-to-br from-emerald-600 to-teal-700 rounded-2xl p-6 text-white shadow-md flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
        ...
        <div class="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
            ...
            <div
                ...
                <div class="bg-slate-100 p-1 rounded-xl border border-slate-200/60 w-full sm:w-auto">
                    <select v-model="selectedBulan"
                        class="bg-transparent text-slate-700 font-semibold text-xs px-3 py-1.5 focus:outline-none w-full cursor-pointer">
                        <!-- Teks bulan diubah menjadi Bahasa Indonesia -->
            ...
            <div class="px-5 py-4 bg-emerald-50/40 border-b border-slate-100 flex justify-between items-center text-sm">
                <span class="text-slate-500 font-medium">Total Terkumpul ({{ formatBulanIndo(selectedBulan) }}):</span>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-50 text-xs">
                        <tr v-for="setor in setoranBulanTerpilih" :key="setor.minggu" class="hover:bg-slate-50/30">
                            ...
                            <!-- Format Tanggal Setoran ke Bahasa Indonesia -->
                            <td class="py-3 px-5 text-slate-600">{{ formatTanggalIndo(setor.tanggal) }}</td>
                            <td class="py-3 px-5 text-right font-bold text-slate-700">{{ formatRupiah(setor.nominal) }}
                            </td>
                        </tr>
                        <tr v-if="setoranBulanTerpilih.length === 0">
                            <td colspan="3" class="py-6 text-center text-slate-400 italic">
                                Belum ada catatan setoran untuk bulan {{ formatBulanIndo(selectedBulan) }}.
                            </td>
                        </tr>
                    </tbody>
                </table>
...
export default {
    ...
    setup(props) {
        ...
        // Fungsi mengubah bulan (misal: "July 2026" -> "Juli 2026")
        ...
        // Fungsi mengubah format tanggal (misal: "12 July 2026" -> "12 Juli 2026")
        const formatTanggalIndo = (tanggalEN) => {
            if (!tanggalEN) return '';
            let hasil = tanggalEN;

            Object.keys(kamusBulan).forEach(enKey => {
                const regex = new RegExp(`\\b${enKey}\\b`, 'gi');
                hasil = hasil.replace(regex, kamusBulan[enKey]);
            });
            return hasil;
        };
        ...
        // 4. CONFIG DATA CHART UNTUK TREN BULANAN (Diterjemahkan ke Bahasa Indonesia)
        ...
        onMounted(async () => {
            ...
            try {
                ...
                if (!json.error) {
                    ...
                    // --- LOGIKA BULAN SAAT INI (BAHASA INGGRIS) ---
                    const daftarNamaBulanEN = [
                        "January", "February", "March", "April", "May", "June",
                        "July", "August", "September", "October", "November", "December"
                    ];
                    const sekarang = new Date();
                    const bulanTahunSekarang = `${daftarNamaBulanEN[sekarang.getMonth()]} ${sekarang.getFullYear()}`; // Contoh: "July 2026"
                    ...
                    if (indexBulanSekarang !== -1) {
                        selectedBulan.value = listBulan.value[indexBulanSekarang];
